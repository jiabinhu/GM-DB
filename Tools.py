    ...
    def readDBName(self):
        ...
        for name in Name_list:
            if len(DB_list) < len(Name_list):
                ...
                break
        GDB_list.append(DB_list)
        GDB_list.append(GB_list)
...
#5.根据输入配置文件的GB_Device、DB_Device初始化字段的列表存储--》批量初始化列表
    def readDevice(self)->list:
        ...
        print(len(GB_Device_list))

        DB_Device_list = filedatas[2][1]
        print(len(DB_Device_list))
        GBDevice = []
        DBDevice = []
...
#7.从JSON里面获取到的数据添加到列表里面-->[['0-2-0-8', '30720', '26624', '48128', '1', '', '', '530', '29', '0', '9', '0', '13334', '100'],[*]]
    def writedata(self)->list:
        ...
        Device = [[] for i in range(data_len+5)]
        # print(data[16])
...
#8.从列表数据输出到excel
    def ToExcel(self):
        ...
        file = open("JSONdata.xlsx", 'a',encoding='GBK')
